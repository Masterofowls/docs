...
- [Media](media) — `<img>`, `<video>`, `<audio>` as drawable sources
- [Script](script) — loading drawing code
- [Style](style) — CSS sizing vs canvas attributes
- [Div](div) — layout wrappers around canvas
- [Section](section) — structuring figure/chart regions
- [URL](url) — blob/object URLs for exports
