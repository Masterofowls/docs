...
Python offers threads, processes, and asyncio for concurrent work. Choose based on the bottleneck: **I/O-bound** → threads or asyncio; **CPU-bound** → multiprocessing (GIL limits CPU threads). See [async.md](async) for `async`/`await` detail.
